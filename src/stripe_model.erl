...
-type error() :: #{error := api_errors(), _ := json:value()}.
%% sku_inventory
-type sku_inventory() ::
    #{value => binary() | null,
      type := binary(),
...
-type payment_method_details_wechat() :: #{_ := json:value()}.
...
-type invoice_payment_method_options_bancontact() ::
    #{preferred_language := de | en | fr | nl, _ := json:value()}.
%% invoice_tax_amount
...
-type payment_method_details_card_wallet_amex_express_checkout() :: #{_ := json:value()}.
...
-type invoice_payment_method_options_acss_debit_mandate_options() ::
    #{transaction_type => business | personal | null, _ := json:value()}.
%% payment_method_eps
...
-type payment_method_boleto() :: #{tax_id := binary(), _ := json:value()}.
...
-type issuing_cardholder_company() :: #{tax_id_provided := boolean(), _ := json:value()}.
%% token
%%
...
-type payment_method_sofort() :: #{country => binary() | null, _ := json:value()}.
%% payment_link
%%
...
-type payment_intent_card_processing() :: #{_ := json:value()}.
%% payment_method_options_grabpay
-type payment_method_options_grabpay() ::
    #{setup_future_usage => none, _ := json:value()}.
%% dispute_evidence
...
-type payment_links_resource_subscription_data() ::
    #{trial_period_days => integer() | null, _ := json:value()}.
...
-type gelato_report_id_number_options() :: #{_ := json:value()}.
...
-type invoice_setting_subscription_schedule_setting() ::
    #{days_until_due => integer() | null, _ := json:value()}.
%% subscription_item
%%
...
-type setup_attempt_payment_method_details_sepa_debit() :: #{_ := json:value()}.
...
-type payment_intent_next_action_verify_with_microdeposits() ::
    #{hosted_verification_url := binary(),
...
-type payment_method_details_boleto() :: #{tax_id := binary(), _ := json:value()}.
%% payment_method_details_card_wallet_apple_pay
-type payment_method_details_card_wallet_apple_pay() :: #{_ := json:value()}.
...
-type subscription_schedule_phase_configuration() ::
    #{trial_end => integer() | null,
      ...
      _ := json:value()}.
%% mandate_multi_use
-type mandate_multi_use() :: #{_ := json:value()}.
...
-type issuing_authorization_amount_details() ::
    #{atm_fee => integer() | null, _ := json:value()}.
%% payment_method_bancontact
-type payment_method_bancontact() :: #{_ := json:value()}.
...
-type setup_attempt_payment_method_details_card() ::
    #{three_d_secure => three_d_secure_details(), _ := json:value()}.
...
-type setup_attempt_payment_method_details_acss_debit() :: #{_ := json:value()}.
%% capability
%%
...
-type schedules_phase_automatic_tax() :: #{enabled := boolean(), _ := json:value()}.
%% discount
%%
...
-type invoice_payment_method_options_card() ::
    #{request_three_d_secure => any | automatic | null, _ := json:value()}.
...
-type issuing_cardholder_address() :: #{address := address(), _ := json:value()}.
...
-type payment_method_card_present() :: #{_ := json:value()}.
%% setup_attempt_payment_method_details_boleto
-type setup_attempt_payment_method_details_boleto() :: #{_ := json:value()}.
...
-type payment_method_options_fpx() :: #{setup_future_usage => none, _ := json:value()}.
...
-type payment_method_card_wallet_samsung_pay() :: #{_ := json:value()}.
%% offline_acceptance
-type offline_acceptance() :: #{_ := json:value()}.
%% plan_tier
-type plan_tier() ::
    #{up_to => integer() | null,
      unit_amount_decimal => binary() | null,
...
-type card_mandate_payment_method_details() :: #{_ := json:value()}.
...
-type payment_method_options_card_present() :: #{_ := json:value()}.
...
-type setup_attempt_payment_method_details_card_present() ::
    #{generated_card => binary() | payment_method(), _ := json:value()}.
%% price_tier
-type price_tier() ::
    #{up_to => integer() | null,
      unit_amount_decimal => binary() | null,
...
-type checkout_boleto_payment_method_options() ::
    #{expires_after_days := integer(), _ := json:value()}.
%% balance_transaction
%%
...
-type payment_flows_private_payment_methods_alipay() :: #{_ := json:value()}.
%% quotes_resource_computed
-type quotes_resource_computed() ::
    #{upfront := quotes_resource_upfront(),
      recurring => quotes_resource_recurring(),
      _ := json:value()}.
...
-type payment_method_options_interac_present() :: #{_ := json:value()}.
%% payment_method_giropay
-type payment_method_giropay() :: #{_ := json:value()}.
...
-type sigma_scheduled_query_run_error() :: #{message := binary(), _ := json:value()}.
...
-type legal_entity_company_verification() ::
    #{document := legal_entity_company_verification_document(), _ := json:value()}.
...
-type account_bacs_debit_payments_settings() ::
    #{display_name => binary(), _ := json:value()}.
...
-type payment_links_resource_completion_behavior_confirmation_page() ::
    #{custom_message => binary() | null, _ := json:value()}.
...
-type three_d_secure_usage() :: #{supported := boolean(), _ := json:value()}.
...
-type payment_method_details_card_wallet_samsung_pay() :: #{_ := json:value()}.
...
-type payment_method_options_giropay() ::
    #{setup_future_usage => none, _ := json:value()}.
%% coupon_applies_to
-type coupon_applies_to() :: #{products := [binary()], _ := json:value()}.
...
-type payment_method_wechat_pay() :: #{_ := json:value()}.
...
-type gelato_session_id_number_options() :: #{_ := json:value()}.
%% payment_method_options_p24
-type payment_method_options_p24() :: #{setup_future_usage => none, _ := json:value()}.
%% mandate_acss_debit
...
-type payment_method_klarna() ::
    #{dob => payment_flows_private_payment_methods_klarna_dob(), _ := json:value()}.
%% ephemeral_key
-type ephemeral_key() ::
    #{secret => binary(),
...
-type payment_method_details_grabpay() ::
    #{transaction_id => binary() | null, _ := json:value()}.
...
-type payment_method_interac_present() :: #{_ := json:value()}.
%% deleted_discount
...
-type payment_method_card_wallet_amex_express_checkout() :: #{_ := json:value()}.
%% account_sepa_debit_payments_settings
-type account_sepa_debit_payments_settings() ::
    #{creditor_id => binary(), _ := json:value()}.
%% portal_invoice_list
-type portal_invoice_list() :: #{enabled := boolean(), _ := json:value()}.
...
-type account_card_issuing_settings() ::
    #{tos_acceptance => card_issuing_account_terms_of_service(), _ := json:value()}.
%% deleted_person
-type deleted_person() ::
    #{object := person,
      id := binary(),
...
-type payment_method_details_oxxo() :: #{number => binary() | null, _ := json:value()}.
...
-type payment_method_grabpay() :: #{_ := json:value()}.
%% source_type_alipay
...
-type payment_intent_next_action_wechat_pay_redirect_to_ios_app() ::
    #{native_url := binary(), _ := json:value()}.
%% charge_outcome
-type charge_outcome() ::
    #{type := binary(),
...
-type portal_payment_method_update() :: #{enabled := boolean(), _ := json:value()}.
...
-type verification_session_redaction() ::
    #{status := processing | redacted, _ := json:value()}.
...
-type payment_method_card_wallet_apple_pay() :: #{_ := json:value()}.
%% legal_entity_dob
...
-type payment_method_details_card_installments() ::
    #{plan => payment_method_details_card_installments_plan(), _ := json:value()}.
%% tax_code
%%
...
-type payment_method_details_stripe_account() :: #{_ := json:value()}.
...
-type payment_method_options_alipay() ::
    #{setup_future_usage => none | off_session, _ := json:value()}.
...
-type portal_subscription_pause() :: #{enabled := boolean(), _ := json:value()}.
%% deleted_coupon
-type deleted_coupon() ::
    #{object := coupon,
      id := binary(),
...
-type issuing_cardholder_verification() ::
    #{document => issuing_cardholder_id_document(), _ := json:value()}.
%% setup_attempt_payment_method_details_au_becs_debit
-type setup_attempt_payment_method_details_au_becs_debit() :: #{_ := json:value()}.
%% portal_features
...
-type legal_entity_company_verification_document() ::
    #{front => binary() | file(),
      ...
      _ := json:value()}.
%% payment_method_oxxo
-type payment_method_oxxo() :: #{_ := json:value()}.
%% setup_intent_payment_method_options_card
-type setup_intent_payment_method_options_card() ::
    #{request_three_d_secure => any | automatic | challenge_only | null, _ := json:value()}.
...
-type subscription_automatic_tax() :: #{enabled := boolean(), _ := json:value()}.
%% period
-type period() ::
    #{start => integer() | null,
      'end' => integer() | null,
...
-type payment_pages_checkout_session_consent_collection() ::
    #{promotions => auto | null, _ := json:value()}.
%% mandate
%%
...
-type payment_method_details_card_wallet_google_pay() :: #{_ := json:value()}.
...
-type payment_method_details_klarna() ::
    #{preferred_locale => binary() | null,
      payment_method_category => binary() | null,
      _ := json:value()}.
%% payment_method_options_bancontact
...
-type discounts_resource_discount_amount() ::
    #{discount := binary() | discount() | deleted_discount(),
      amount := integer(),
      _ := json:value()}.
...
-type account_requirements_alternative() ::
    #{original_fields_due := [binary()],
      alternative_fields_due := [binary()],
      _ := json:value()}.
...
-type address() ::
    #{state => binary() | null,
      ...
      _ := json:value()}.
%% source_type_p24
-type source_type_p24() :: #{reference => binary() | null, _ := json:value()}.
...
-type source_mandate_notification_acss_debit_data() ::
    #{statement_descriptor => binary(), _ := json:value()}.
...
-type payment_method_card_wallet_google_pay() :: #{_ := json:value()}.
%% tax_id
%%
...
-type usage_record_summary() ::
    #{total_usage := integer(),
      ...
      _ := json:value()}.
%% balance_detail
-type balance_detail() :: #{available := [balance_amount()], _ := json:value()}.
%% transfer_data
-type transfer_data() ::
    #{destination := binary() | account(),
      amount => integer(),
...
-type payment_method_afterpay_clearpay() :: #{_ := json:value()}.
%% source_order
-type source_order() ::
    #{shipping => shipping(),
...
-type payment_method_details_afterpay_clearpay() ::
    #{reference => binary() | null, _ := json:value()}.
%% notification_event_data
-type notification_event_data() ::
    #{previous_attributes => json:value(),
      object := json:value(),
      _ := json:value()}.
%% person
%%
...
-type mandate_au_becs_debit() :: #{url := binary(), _ := json:value()}.
%% payment_pages_checkout_session_consent
-type payment_pages_checkout_session_consent() ::
    #{promotions => opt_in | opt_out | null, _ := json:value()}.
%% setup_attempt_payment_method_details_bacs_debit
-type setup_attempt_payment_method_details_bacs_debit() :: #{_ := json:value()}.
%% online_acceptance
...
-type subscription_item_billing_thresholds() ::
    #{usage_gte => integer() | null, _ := json:value()}.
%% payment_method_options_klarna
