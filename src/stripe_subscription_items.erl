...
-module(stripe_subscription_items).

-export([get_all/2]).

-type get_all_options() ::
        stripe_client:get_subscription_items_request_query().

-spec get_all(get_all_options(), stripe:client_options()) ->
        stripe:result([stripe_model:subscription_item()]).
...
-spec get_all(get_all_options(), stripe_client:options(),
               [stripe_model:subscription_item()]) ->
        stripe:result([stripe_model:subscription_item()]).
get_all(Options, ClientOptions, Acc) ->
  ReqOptions = #{query => Options},
  case stripe_client:get_subscription_items(ReqOptions, ClientOptions) of
    {ok, #{data := Items, has_more := More}, #{status := S}} when
        S >= 200, S < 300 ->
      Acc2 = [Items | Acc],
      case More of
        true ->
          #{id := LastId} = lists:last(Items),
